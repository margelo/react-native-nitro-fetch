///
/// NitroFormDataPart.hpp
/// This file was generated by nitrogen. DO NOT MODIFY THIS FILE.
/// https://github.com/mrousavy/nitro
/// Copyright © 2026 Marc Rousavy @ Margelo
///

#pragma once

#if __has_include(<NitroModules/JSIConverter.hpp>)
#include <NitroModules/JSIConverter.hpp>
#else
#error NitroModules cannot be found! Are you sure you installed NitroModules properly?
#endif
#if __has_include(<NitroModules/NitroDefines.hpp>)
#include <NitroModules/NitroDefines.hpp>
#else
#error NitroModules cannot be found! Are you sure you installed NitroModules properly?
#endif



#include <string>
#include <optional>

namespace margelo::nitro::nitrofetch {

  /**
   * A struct which can be represented as a JavaScript object (NitroFormDataPart).
   */
  struct NitroFormDataPart {
  public:
    std::string name     SWIFT_PRIVATE;
    std::optional<std::string> value     SWIFT_PRIVATE;
    std::optional<std::string> fileUri     SWIFT_PRIVATE;
    std::optional<std::string> fileName     SWIFT_PRIVATE;
    std::optional<std::string> mimeType     SWIFT_PRIVATE;

  public:
    NitroFormDataPart() = default;
    explicit NitroFormDataPart(std::string name, std::optional<std::string> value, std::optional<std::string> fileUri, std::optional<std::string> fileName, std::optional<std::string> mimeType): name(name), value(value), fileUri(fileUri), fileName(fileName), mimeType(mimeType) {}
  };

} // namespace margelo::nitro::nitrofetch

namespace margelo::nitro {

  // C++ NitroFormDataPart <> JS NitroFormDataPart (object)
  template <>
  struct JSIConverter<margelo::nitro::nitrofetch::NitroFormDataPart> final {
    static inline margelo::nitro::nitrofetch::NitroFormDataPart fromJSI(jsi::Runtime& runtime, const jsi::Value& arg) {
      jsi::Object obj = arg.asObject(runtime);
      return margelo::nitro::nitrofetch::NitroFormDataPart(
        JSIConverter<std::string>::fromJSI(runtime, obj.getProperty(runtime, "name")),
        JSIConverter<std::optional<std::string>>::fromJSI(runtime, obj.getProperty(runtime, "value")),
        JSIConverter<std::optional<std::string>>::fromJSI(runtime, obj.getProperty(runtime, "fileUri")),
        JSIConverter<std::optional<std::string>>::fromJSI(runtime, obj.getProperty(runtime, "fileName")),
        JSIConverter<std::optional<std::string>>::fromJSI(runtime, obj.getProperty(runtime, "mimeType"))
      );
    }
    static inline jsi::Value toJSI(jsi::Runtime& runtime, const margelo::nitro::nitrofetch::NitroFormDataPart& arg) {
      jsi::Object obj(runtime);
      obj.setProperty(runtime, "name", JSIConverter<std::string>::toJSI(runtime, arg.name));
      obj.setProperty(runtime, "value", JSIConverter<std::optional<std::string>>::toJSI(runtime, arg.value));
      obj.setProperty(runtime, "fileUri", JSIConverter<std::optional<std::string>>::toJSI(runtime, arg.fileUri));
      obj.setProperty(runtime, "fileName", JSIConverter<std::optional<std::string>>::toJSI(runtime, arg.fileName));
      obj.setProperty(runtime, "mimeType", JSIConverter<std::optional<std::string>>::toJSI(runtime, arg.mimeType));
      return obj;
    }
    static inline bool canConvert(jsi::Runtime& runtime, const jsi::Value& value) {
      if (!value.isObject()) {
        return false;
      }
      jsi::Object obj = value.getObject(runtime);
      if (!JSIConverter<std::string>::canConvert(runtime, obj.getProperty(runtime, "name"))) return false;
      if (!JSIConverter<std::optional<std::string>>::canConvert(runtime, obj.getProperty(runtime, "value"))) return false;
      if (!JSIConverter<std::optional<std::string>>::canConvert(runtime, obj.getProperty(runtime, "fileUri"))) return false;
      if (!JSIConverter<std::optional<std::string>>::canConvert(runtime, obj.getProperty(runtime, "fileName"))) return false;
      if (!JSIConverter<std::optional<std::string>>::canConvert(runtime, obj.getProperty(runtime, "mimeType"))) return false;
      return true;
    }
  };

} // namespace margelo::nitro
