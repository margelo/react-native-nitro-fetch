...
open class HybridNitroCronetSpec_cxx {
  ...
  @inline(__always)
  public final func fetchSync(url: std.string, httpMethod: std.string, headers: bridge.std__unordered_map_std__string__std__string_, body: bridge.std__optional_std__string_) -> bridge.Result_SyncFetchResponse_ {
    do {
      let __result = try self.__implementation.fetchSync(url: String(url), httpMethod: String(httpMethod), headers: { () -> Dictionary<String, String> in
        var __dictionary = Dictionary<String, String>(minimumCapacity: headers.size())
        let __keys = bridge.get_std__unordered_map_std__string__std__string__keys(headers)
        for __key in __keys {
          let __value = bridge.get_std__unordered_map_std__string__std__string__value(headers, __key)
      ...
      return bridge.create_Result_SyncFetchResponse_(__resultCpp)
    } catch (let __error) {
      let __exceptionPtr = __error.toCpp()
      return bridge.create_Result_SyncFetchResponse_(__exceptionPtr)
    }
  }
