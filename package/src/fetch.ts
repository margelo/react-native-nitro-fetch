...
export const boxedNitroCronet = NitroModules.box(NitroCronet);
...
export async function prefetchOnAppStart(
  ...
  const { NativeStorage } = require('./NitroInstances');
  ...
  storage.setString(KEY, newQueueStr);
}
...
export async function removeFromAutoPrefetch(
  ...
  if (next.length === 0) {
    ...
    storage.setString(KEY, JSON.stringify(next));
  }
}
...
export async function clearAutoPrefetchQueue(): Promise<void> {
  ...
  storage.setString(KEY, JSON.stringify([]));
}

// Optional off-thread processing using react-native-worklets-core
export interface WorkletFetchResponse {
  url: string;
  status: number;
  statusText: string;
  ok: boolean;
...
export type WorkletMapper<T> = (response: WorkletFetchResponse) => T | Promise<T>;
...
export async function fetchOnWorklet<T>(
  ...
  if (!rt || !Worklets || (typeof rt.run !== 'function' && typeof rt.runAsync !== 'function')) {
  ...
  return await rt.runAsync(() => {
    'worklet';
    const unboxed = boxedNitroCronet.unbox();
    const method = options?.method || 'GET';
    const headers = options?.headers || {};
    let bodyRaw = options?.body;

    // fetchSync only accepts string body — convert ArrayBuffer if needed
    const bodyStr = typeof bodyRaw === 'string'
      ? bodyRaw
      : undefined;

    const res: SyncFetchResponse = unboxed.fetchSync(url, method, headers, bodyStr);
    const workletRes: WorkletFetchResponse = {
      url: res.url,
      status: res.status,
      statusText: res.statusText,
    ...
    console.log(workletRes);
    return mapWorklet(workletRes);
  })
}
