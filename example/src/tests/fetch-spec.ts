...
test(SUITE, 'cache: default uses HTTP cache (faster second time)', async () => {
  // Use unique URL to ensure no prior cache
  const url = getServerUrl('/data/10kb') + `?test=cache_${Date.now()}`;
  ...
  console.log(`First request (network): ${duration1.toFixed(2)}ms`);

  // Make 50 cached requests - all should be consistently fast
  ...
  console.log(`Cached requests (avg of 50): ${avgCached.toFixed(2)}ms`);
  console.log(`Cached requests (min): ${minCached.toFixed(2)}ms`);
  console.log(`Cached requests (max): ${maxCached.toFixed(2)}ms`);
  console.log(`Cache speedup: ${(duration1 / avgCached).toFixed(1)}x`);

  // All cached requests should be consistently faster (at least 2x)
  expect(avgCached).to.be.lessThan(duration1 / 2);
  expect(maxCached).to.be.lessThan(duration1 / 2);
});

test(SUITE, 'cache: no-cache bypasses HTTP cache (same speed)', async () => {
  // Use unique URL
  const url = getServerUrl('/data/10kb') + `?test=nocache_${Date.now()}`;

  // Make 50 requests with no-cache - all should hit network
  ...
  console.log(`No-cache requests (avg of 50): ${avgNoCache.toFixed(2)}ms`);
  console.log(`No-cache requests (min): ${minNoCache.toFixed(2)}ms`);
  console.log(`No-cache requests (max): ${maxNoCache.toFixed(2)}ms`);

  // Key test: Average no-cache time should be significantly slower than what
  // we'd expect from HTTP cache (which would be ~10-15ms based on Test 1)
  expect(avgNoCache).to.be.greaterThan(20); // Much slower than cache

  console.log(
    `✅ No-cache correctly bypasses HTTP cache (avg ${avgNoCache.toFixed(2)}ms vs expected cache ~10-15ms)`
  );
});

test(SUITE, 'cache: mixed - default then no-cache', async () => {
  const url = getServerUrl('/data/10kb') + `?test=mixed_${Date.now()}`;

  // 1. Initial network request to populate cache
  const start1 = performance.now();
  const response1 = await fetch(url);
  const duration1 = performance.now() - start1;
  await response1.text();
  console.log(`Initial request (network): ${duration1.toFixed(2)}ms`);

  // 2. Make 25 cached requests - should all be fast
  const cachedTimes: number[] = [];
  for (let i = 0; i < 25; i++) {
    const start = performance.now();
    const response = await fetch(url);
  ...
  const minCached = Math.min(...cachedTimes);
  const maxCached = Math.max(...cachedTimes);
  console.log(`Cached requests (avg of 25): ${avgCached.toFixed(2)}ms`);
  console.log(`Cached requests (min): ${minCached.toFixed(2)}ms`);
  console.log(`Cached requests (max): ${maxCached.toFixed(2)}ms`);

  // 3. Make 25 no-cache requests - should all hit network
  const noCacheTimes: number[] = [];
  for (let i = 0; i < 25; i++) {
    const start = performance.now();
    const response = await fetch(url, { cache: 'no-cache' });
  ...
  const minNoCache = Math.min(...noCacheTimes);
  const maxNoCache = Math.max(...noCacheTimes);
  console.log(`No-cache requests (avg of 25): ${avgNoCache.toFixed(2)}ms`);
  console.log(`No-cache requests (min): ${minNoCache.toFixed(2)}ms`);
  console.log(`No-cache requests (max): ${maxNoCache.toFixed(2)}ms`);

  // The key test: Cached requests should be MUCH faster than no-cache requests
  // This proves that no-cache is bypassing the HTTP cache layer
  expect(avgCached).to.be.lessThan(avgNoCache / 2);

  console.log(
    `Cache vs no-cache speedup: ${(avgNoCache / avgCached).toFixed(1)}x`
  );

  // Note: no-cache may be faster than initial request due to TCP connection pooling,
  // DNS caching, and TLS session resumption - this is expected and correct!
});
