...
class NitroCronet : HybridNitroCronetSpec() {
  ...
  override fun fetchSync(
    ...
    if (!prefetchKey.isNullOrEmpty()) {
      ...
      if (cached != null) {
        ...
        return SyncFetchResponse(
          ...
          statusText = cached.statusText,
          headers = headersMap,
          body = String(cached.body, Charsets.UTF_8)
        )
      }
      ...
      if (pendingFuture != null) {
        return try {
          val cachedResponse = pendingFuture.get()
          val headersMap = cachedResponse.headers.toMutableMap()
          headersMap["nitroPrefetched"] = "true"
          val byteBuffer = java.nio.ByteBuffer.allocateDirect(cachedResponse.body.size)
          byteBuffer.put(cachedResponse.body)
          byteBuffer.flip()
          val arrayBuffer = ArrayBuffer.wrap(byteBuffer)
          SyncFetchResponse(
            url = cachedResponse.url,
            status = cachedResponse.statusCode.toDouble(),
            statusText = cachedResponse.statusText,
            headers = headersMap,
            body = String(cachedResponse.body, Charsets.UTF_8)
          )
        } catch (e: Exception) {
          throw e.cause ?: e
        }
    ...
    var result: SyncFetchResponse? = null
      ...
      NitroFetchHelper.simpleFetch(
        ...
        onSuccess = { response ->
          result = SyncFetchResponse(
              url = response.url,
              status = response.statusCode.toDouble(),
              statusText = response.statusText,
              headers = response.headers,
              body = String(response.body, Charsets.UTF_8)
            )
          latch.countDown()
        },
        onFail = { t ->
